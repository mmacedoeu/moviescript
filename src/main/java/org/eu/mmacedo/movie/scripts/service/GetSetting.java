...
public class GetSetting implements Function<Long, Optional<JSONObject>> {
	...
	public final Function<Long, JSONArray> getWordCounts = id -> {
	...
	public final Function<Collection<MovieCharacter>, JSONArray> getMovieCharacter = c -> {
		final JSONArray jsonArray = new JSONArray();
	...
	public Optional<JSONObject> apply(final Long arg0) {
		final JSONObject jsonObject = new JSONObject();
		final Optional<Setting> one = settingRepository.findById(arg0);
		if (one.isPresent()) {
			final Setting setting = one.get();

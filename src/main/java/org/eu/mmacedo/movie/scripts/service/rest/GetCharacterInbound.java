...
public class GetCharacterInbound implements Function<Message, Message> {
	@Autowired
	...
	public Message apply(final Message arg0) {
		...
		final String id = (String) arg0.getHeaders().get("c");
		try {
			final Long valueOf = Long.valueOf(id);
			final Optional<JSONObject> apply = getCharacterGateway.apply(valueOf);
			...
			if (receive != null && receive.getPayload() != null) {
				...
				if (apply.isPresent()) {
					...
					final JSONObject jsonObject = new JSONObject();
					jsonObject.put("message", "Movie character with id " + id + " not found");
